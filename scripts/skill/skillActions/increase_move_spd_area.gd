class_name SkillActionIncreaseMoveSpdArea
extends SkillAction
...
func enemyEntered(area: Area2D):
	...
	if(area.has_method("addIncreaseMoveSpeed")):
		area.addIncreaseMoveSpeed(increaseValue, getDebuffKey());
...
func enemyExited(area: Area2D):
	...
	if(area.has_method("removeIncreaseMoveSpeed")):
		area.removeIncreaseMoveSpeed(getDebuffKey());
