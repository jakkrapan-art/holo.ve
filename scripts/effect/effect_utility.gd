...
static func make_instance(effect_id: String, source_id: String, value: float, duration: float, applier: Node = null, authored_title: String = "") -> EffectInstance:
	...
	# surface in the synergy panel, not as a per-unit status icon - a default;
	# an earned per-cast synergy buff may override show_icon after build
	# (SynergyEffectRobotic).
